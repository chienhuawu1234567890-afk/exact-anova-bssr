...
# =========================================================
# 3-Group (k=3) Blinded SSR Summaries & Figures
# Keep only:
...
target_power  <- 0.80
sigma_true    <- 10
mu_design     <- c(0, 5, 10)   # planned means for 3 groups
out_dir       <- "outputs_ssr_g3"
R             <- 10000
...
# MSE                = SSE*/(N*-g)
# adjusted_variance  = [SSTO* - sum_i m_i*(mu_i-mu_w)^2]/(N*-1)
...
compute_interim_estimators <- function(y_mat, mu_vec, m_vec = NULL) {
  ...
  adjusted_variance <- (SSTO_star - correction) / (N_star - 1)
...
for (th in thetas) {
  ...
  write.csv(
    bias_mse_table,
    file.path(out_dir, paste0("variance_bias_mse_", tag, ".csv")),
    row.names = FALSE
  )
  
  print(paste("Theta =", th))
  print(bias_mse_table)
}
...
for (th in thetas) {
  ...
  n_methods_clean   <- lapply(n_methods, function(x) x[is.finite(x) & !is.na(x)])
  ...
  write.csv(dist_var, file.path(out_dir, paste0("dist_variance_", tag, ".csv")), row.names = TRUE)
  write.csv(dist_n, file.path(out_dir, paste0("dist_n_", tag, ".csv")), row.names = TRUE)
  write.csv(dist_power, file.path(out_dir, paste0("dist_power_", tag, ".csv")), row.names = TRUE)
...
for (th in thetas) {
  ...
  print(bias_mse_table, row.names = FALSE)
  ...
  # ---- 3. Bias of re-estimated sample sizes ----
  n_plan_true <- initial_n_plan(
    mu_vec = mu_design,
    sigma = sigma_true,
    k = k_groups,
    alpha = alpha,
  ...
  bias_n_table <- data.frame(
    Estimator = c("n_star", "n_star2", "n_star3"),
    Bias = c(
      mean(nres$n_star, na.rm = TRUE)  - n_plan_true,
      mean(nres$n_star2, na.rm = TRUE) - n_plan_true,
      mean(nres$n_star3, na.rm = TRUE) - n_plan_true
    )
  )
  
  cat("\nTable 4: Bias of Re-estimated Sample Sizes\n")
  print(bias_n_table, row.names = FALSE)
  
  # ---- 4. MSE of re-estimated sample sizes ----
  mse_n_table <- data.frame(
    Estimator = c("n_star", "n_star2", "n_star3"),
    MSE = c(
      mean((nres$n_star  - n_plan_true)^2, na.rm = TRUE),
      mean((nres$n_star2 - n_plan_true)^2, na.rm = TRUE),
      mean((nres$n_star3 - n_plan_true)^2, na.rm = TRUE)
    )
  )
  
  cat("\nTable 5: MSE of Re-estimated Sample Sizes\n")
  print(mse_n_table, row.names = FALSE)
  
  # ---- 5. Distribution summaries for re-estimated sample sizes ----
  n_methods_clean <- lapply(n_results[[tag]], function(x) x[is.finite(x) & !is.na(x)])
  dist_n <- do.call(rbind, lapply(n_methods_clean, summarize_distribution))
  rownames(dist_n) <- names(n_methods_clean)
